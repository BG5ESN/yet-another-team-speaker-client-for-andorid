...
### v2.1.5 → v2.1.22（2026-09-11 ~ 09-13）语音通话专项

> 这两天连发了 18 个版本，绝大多数是围绕通话的缺陷修复与打磨，版本号本身没有用户
> 可感的意义。下面按**主题**归并，版本号标在每条末尾，只为对照提交历史。
> 两次做成又撤回的尝试单列在最后，不是更新内容，是给后来人省点弯路。

**稳定性 —— 修偶发闪退**（2.1.11）

- `JitterBuffer` 的 `TreeMap` 被两条线程并发访问（写包在 service 主线程、读改在音频
  播放线程），红黑树被写坏后在 `fixAfterInsertion` 抛 NPE。加锁 + 计数器 `@Volatile`，
  并补并发回归测试（做过对照实验：解除锁即复现）

**音频链路**

- 抖动缓冲 + 丢包隐藏：按语音包号去重 / 重排 / PLC 补帧，抗网络抖动（2.1.11）
- 播放时钟改为单调时间基：按绝对 20ms 时隙推进，不再依赖 `playbackHeadPosition`
  —— 部分机型该值恒为 0，会让缓冲判断失效、彻底没声音（2.1.5）
- 语音激活门控（VA）真正作用于发送：此前是"假门控"，一直按 50 包/秒上行（2.1.12 前后）
- 说话圈判据回到「流判定」：远端收到语音包即点亮，去掉解码能量那一层，避免抖动缓冲
  预填充带来的滞后（2.1.12）
- 修抖动缓冲深度锁死：升档判据误用 PLC 计数形成正反馈，深度顶到上限再不回落；上限同步
  放宽到 240ms（2.1.11 前后）

**输出设备与音频焦点**

- 新增通话音频输出设备选择：顶栏菜单切换扬声器 / 听筒 / 有线 / 蓝牙，列表随插拔实时刷新，
  记住选择，设备消失自动回退「跟随系统」（2.1.17）
- 蓝牙 A2DP 与 SCO 两条通道分别列出（前者音质好、后者麦克风可用）（2.1.17）
- 采集侧按输入设备列表匹配，不再把输出设备硬塞给 `AudioRecord`（2.1.19）
- 设备列表排掉 `TYPE_FM`（收音机）与 `TYPE_TELEPHONY`（虚拟设备）（2.1.19）
- 新增「通话独占声音」开关：开启抢占音频焦点（音乐暂停），关闭则与音乐并存（2.1.18）
- 修被音乐抢走焦点后**永久静音**：`AUDIOFOCUS_LOSS` 是永久失去，系统不会再回调
  `GAIN`，必须自己重抢（2.1.14）
- 音量增益可在通话界面底栏直接调，拖动实时生效（2.1.20）

**通话交互**

- 新增全屏触摸 PTT：双指唤出，任意位置按住说话，期间屏幕常亮（2.1.9）
- 悬浮窗若干修复：卡在最后说话的人身上、圆形头像按下露方框、说话人切换延迟叠加
  导致 icon 要 540ms 才反应（2.1.7 / 2.1.8 / 2.1.13）

**可观测性**

- 崩溃取证：`Thread.UncaughtExceptionHandler` + `ApplicationExitInfo` 补记；毫秒级
  时间戳防止同名报告互相覆盖；原生 trace 从 protobuf 二进制里提取可读片段
  （此前直接按 UTF-8 转，67% 是乱码）（2.1.6）
- 事件通道丢弃留痕（计数 + 告警日志），消息消费移出主线程（2.1.6）
- 设置页新增麦克风 VA 测试：按住测电平，看它落在门限哪一侧，带峰值保持游标（2.1.12 前后）

**内部化裁剪**（2.1.5）

- 移除二次元壁纸 / 自定义背景整套、内部化「关于」页、移除应用内检查更新、去掉密聊；
  应用名改为 TS3（详见上方「已移除的功能」）

**试过但不可行（已撤回，不是现存功能）**

- **动态 duck 焦点让音乐变轻**：请求 `AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK` 后讲话没声音。
  `MAY_DUCK` 的字面语义是「我接受被别人压低」，方向搞反了 —— 我们反而成了可被让路的一方。
  （2.1.15 加入，2.1.16 撤回）
- **输出通道（用通信策略隔离音乐）**：设想把音频从 `STRATEGY_MEDIA` 换到 `STRATEGY_PHONE`
  走独立 output thread，从而切输出设备不牵连音乐。实测无效 —— 该机型
  `mCommunicationStrategyId` 为 0，厂商把通信策略并入了媒体策略，音频仍落在同一条
  mixer thread 上。收益为零、代价（音质变窄、无 A2DP）照旧。（2.1.21 加入，2.1.22 撤回）
